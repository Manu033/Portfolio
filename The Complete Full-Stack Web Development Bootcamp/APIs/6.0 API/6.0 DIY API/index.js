...
app.put("/jokes/:id", (req, res)=>{
  const id = parseInt(req.params.id);
  const actualizacionJoke={
    id: id,
    jokeText: req.body.text,
    jokeType: req.body.type,
  };
  const searchIndex =jokes.findIndex((joke)=>joke.id === id);
  jokes[searchIndex]=actualizacionJoke;
  res.json(actualizacionJoke);
});
...
app.delete("/jokes/:id", (req, res)=>{
  ...
  if(searchIndex > -1){
    ...
    res
      .status(400)
      .json({
       error: `El chiste con el id ${id} no se encontro. Por lo que no se pudo borrar.`
      });
  }
...
app.delete("/all", (req, res)=>{
  const llaveDesbloquear = req.query.key;
  if(llaveDesbloquear === masterKey){
    jokes=[];
    res.sendStatus(200);
  }else{
    res
      .status(400) 
      .json({
      error: `No tienes la autorizacion. Por lo que no se pudo borrar.`
      });
  }
});
